...
def nostr_bridge_thread():
    if request.method == "POST":
        ...
        data = nostr_bridge_simple_test(privatedm=False)
        return jsonify(data)
...
def nostr_bridge_simple_test(privatedm = False):
    ...
    # Test Payload
    # job expires in 1 hour
    fileurl ="https://files.catbox.moe/voxrao.wav"
    #fileurl = "https://www.fit.vutbr.cz/~motlicek/sympatex/f2bjrop1.0.wav"

    expiration = str(int((datetime.datetime.now() + datetime.timedelta(minutes=60)).timestamp()))
    event = Event("Transcribe the attached audio file")
    event.kind = 68001
    event.add_tag('j', ["speech-to-text", "whisperx-base"])
    #event.add_tag('params', ["range", "0", "5"]) #this is currently not working on nova server
    event.add_tag('params', ["alignment", "segment"])
    event.add_tag('input', [fileurl, "url"])
    event.add_tag('relays', ["wss://nostr-pub.wellorder.net", "wss://relay.damus.io"])
    event.add_tag('bid', ["1000", "10000"])
    event.add_tag('exp', expiration)
    event.add_tag('p', str(pubkey.hex()))
